...
async def show_stats_start(message: Message, state: FSMContext):
    ...
    builder.button(text="Подробная статистика", callback_data="stats_detailed")
    builder.button(text="❌ Отмена", callback_data="cancel")
    builder.adjust(1)
    await message.answer("📊 Выберите тип статистики:", reply_markup=builder.as_markup())
    await state.set_state(States.choosing_stats_type)

@dp.callback_query(F.data.in_(["stats_general", "stats_detailed"]))
async def choose_stats_period(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    stats_type = callback.data.split("_")[1]
    await state.update_data(stats_type=stats_type)
    builder = InlineKeyboardBuilder()
    current_month = datetime.now().strftime("%Y-%m")
    builder.button(text="Текущий месяц", callback_data=f"period_{current_month}")
    for i in range(1, 12):
        month = (datetime.now() - timedelta(days=30 * i)).strftime("%Y-%m")
        builder.button(text=month, callback_data=f"period_{month}")
    builder.button(text="За год", callback_data="period_year")
    builder.button(text="За все время", callback_data="period_all")
    builder.button(text="❌ Отмена", callback_data="cancel")
    builder.adjust(3)
...
async def show_stats(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        if period == "all":
            ...
            trans_filter = f" AND to_char(CAST(date AS timestamp), 'YYYY') = '{year}'"
            debt_filter = trans_filter
            title = f"за {year} год"
        else:
            trans_filter = f" AND to_char(CAST(date AS timestamp), 'YYYY-MM') = '{period}'"
            debt_filter = trans_filter
            title = f"за {period}"
        with conn.cursor() as cur:
            # Общая статистика
            ...
            cur.execute(f"""
                ...
                FROM debts
                WHERE user_id=%s {debt_filter}
            """, (uid,))
            debt_row = cur.fetchone()
            debt = debt_row['debt_sum']
            # Подробная
            cur.execute(f"""
                SELECT category, SUM(amount) AS sum
                FROM transactions
                WHERE user_id=%s AND type='income' {trans_filter}
                GROUP BY category
                ORDER BY sum DESC
            """, (uid,))
            ...
            cur.execute(f"""
                SELECT category, SUM(amount) AS sum
                FROM transactions
                WHERE user_id=%s AND type='expense' {trans_filter}
                GROUP BY category
                ORDER BY sum DESC
            """, (uid,))
            expense_details = cur.fetchall()
            cur.execute(f"""
                SELECT debtor, amount, description, date
                FROM debts
                WHERE user_id=%s {debt_filter}
                ORDER BY date DESC
            """, (uid,))
        ...
        if stats_type == "detailed":
            if income_details:
                ...
                for d in income_details:
                    text += f"• {d['category']}: {d['sum']:.0f} сўм\n"
                text += "\n"
            if expense_details:
                ...
                text += "\n"
            if debt_details:
                text += "<b>🤝 Долги (подробно):</b>\n"
                for d in debt_details:
                    text += f"• {d['description']} {d['debtor']}: {d['amount']:+.0f} сўм ({d['date']})\n"
